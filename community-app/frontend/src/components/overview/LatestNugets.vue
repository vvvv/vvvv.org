...
<template>
    <div class="section pl-4 nugets">
        <SectionTitle :showRefresh="true" :loading="loading" title="Latest #VL Packs" @sync="sync(true)" link="/packs" :isExternal="true" target="_self"/>
        <div class="row pt-1">
                ...
                <ul v-if="store.nugets.length > 0" class="list-group list-group-flush list">
                    <template v-for="(nuget, index) in store.nugets" :key="index">
                        <li class="list-group-item d-flex flex-nowrap link align-items-baseline">
                            <NImage preview-disabled objectFit="contain" :src="nuget.icon || defaultLogo" :height="25" class="align-self-center" :class="{ 'noImageBack': nuget.icon !== null }"/>
